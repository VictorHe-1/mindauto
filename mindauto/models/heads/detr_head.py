...
from common import build_activation_layer
from mindauto.losses import build_loss
from mindauto.core.utils import multi_apply
from mindauto.core.bbox.assigners import build_assigner
...
def dynamic_clip_for_onnx(x1, y1, x2, y2, max_shape):
    ...
    Since ops.clamp cannot have dynamic `min` and `max`, we scale the
    ...
    Args:
        ...
        max_shape (Tensor): The (H,W) of original image.
    Returns:
        tuple(Tensor): The clipped x1, y1, x2, y2.
    """
    assert isinstance(
        max_shape,
        ms.Tensor), '`max_shape` should be tensor of (h,w) for onnx'
...
class DETRHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is DETRHead):
            ...
            class_weight = ops.ones(num_classes + 1) * class_weight
    ...
    def _init_layers(self):
        ...
        self.fc_cls = nn.Dense(self.embed_dims, self.cls_out_channels)
        self.reg_ffn = FFN(
            self.embed_dims,
            self.embed_dims,
            self.num_reg_fcs,
            ...
            dropout=0.0,
            add_residual=False)
        self.fc_reg = nn.Dense(self.embed_dims, 4)
    ...
    def construct(self, feats, img_metas):
        """Forward function.
    ...
    def forward_single(self, x, img_metas):
        ...
        masks = ops.interpolate(
            masks.unsqueeze(1), size=x.shape[-2:]).astype(ms.int32).squeeze(1)  # TODO: original type torch.bool
        # position encoding
    ...
    def loss_single(self,
        ...
        for img_meta, bbox_pred in zip(img_metas, bbox_preds):
            ...
            factor = ms.Tensor([img_w, img_h, img_w,
                                img_h], dtype=bbox_pred.dtype).unsqueeze(0).tile(
                (bbox_pred.shape[0], 1))
            factors.append(factor)
        factors = ops.cat(factors, 0)
    ...
    def _get_target_single(self,
        ...
        # label targets
        labels = ops.full((num_bboxes,), self.num_classes, dtype=gt_bboxes.dtype)
        labels[pos_inds] = gt_labels[sampling_result.pos_assigned_gt_inds]
        label_weights = gt_bboxes.new_ones(num_bboxes)

        # bbox targets
        bbox_targets = ops.zeros_like(bbox_pred)
        ...
        factor = ms.Tensor([img_w, img_h, img_w,
                            img_h], dtype=bbox_pred.dtype).unsqueeze(0)
    ...
    def _get_bboxes_single(self,
        ...
        if self.loss_cls.use_sigmoid:
            ...
            scores, indexes = cls_score.view(-1).topk(max_per_img)  # TODO: topk return 2 values?
            det_labels = indexes % self.num_classes
            bbox_index = indexes // self.num_classes
            bbox_pred = bbox_pred[bbox_index]
        else:
            scores, det_labels = ops.softmax(cls_score, axis=-1)[..., :-1].max(-1)
        ...
        det_bboxes[:, 0::2] = det_bboxes[:, 0::2].clamp(min=0, max=img_shape[
            1])  # replace det_bboxes[:, 0::2].clamp_(min=0, max=img_shape[1])
        det_bboxes[:, 1::2] = det_bboxes[:, 1::2].clamp(min=0, max=img_shape[0])
        if rescale:
            det_bboxes /= scale_factor
        det_bboxes = ops.cat((det_bboxes, scores.unsqueeze(1)), -1)
    ...
    def simple_test_bboxes(self, feats, img_metas, rescale=False):
        ...
        outs = self.construct(feats, img_metas)
    ...
    def forward_single_onnx(self, x, img_metas):
        ...
        # interpolate masks to have the same spatial shape with x
        masks = ops.interpolate(
            masks.unsqueeze(1), size=x.shape[-2:]).astype(ms.int32).squeeze(1)  # ms.bool
    ...
    def onnx_export(self, all_cls_scores_list, all_bbox_preds_list, img_metas):
        ...
        batch_index_offset = ops.arange(batch_size) * max_per_img
        batch_index_offset = batch_index_offset.unsqueeze(1).broadcast_to((
        ...
        if self.loss_cls.use_sigmoid:
            ...
            scores, det_labels = ops.softmax(
                cls_scores, axis=-1)[..., :-1].max(-1)
        ...
        img_shape_tensor = img_shape.flip(0).tile(2)  # [w,h,w,h]
        img_shape_tensor = img_shape_tensor.unsqueeze(0).unsqueeze(0).broadcast_to((
            batch_size, det_bboxes.shape[1], 4))
        det_bboxes = det_bboxes * img_shape_tensor
        # dynamically clip bboxes
        x1, y1, x2, y2 = det_bboxes.split((1, 1, 1, 1), axis=-1)
        x1, y1, x2, y2 = dynamic_clip_for_onnx(x1, y1, x2, y2, img_shape)
        det_bboxes = ops.cat([x1, y1, x2, y2], axis=-1)
        det_bboxes = ops.cat((det_bboxes, scores.unsqueeze(-1)), -1)
