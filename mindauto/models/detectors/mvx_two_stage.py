...
class MVXTwoStageDetector(Base3DDetector):
    ...
    # TODO: no_grad @mindspore.no_grad()
    def voxelize(self, points):
        """Apply dynamic voxelization to points.

        Args:
            points (list[mindspore.Tensor]): Points of each sample.

        Returns:
            tuple[mindspore.Tensor]: Concatenated points, number of points
        ...
        voxels = ops.cat(voxels, axis=0)
        num_points = ops.cat(num_points, axis=0)
        coors_batch = []
        for i, coor in enumerate(coors):
            coor_pad = ops.pad(coor, (1, 0), mode='constant', value=i)
            coors_batch.append(coor_pad)
        coors_batch = ops.cat(coors_batch, axis=0)
    ...
    def forward_img_train(self,
        ...
        Args:
            x (list[mindspore.Tensor]): Image features of shape (B, C, H, W)
                of multiple levels.
            img_metas (list[dict]): Meta information of images.
            gt_bboxes (list[mindspore.Tensor]): Ground truth boxes of each image
                sample.
            gt_labels (list[mindspore.Tensor]): Ground truth labels of boxes.
            gt_bboxes_ignore (list[mindspore.Tensor], optional): Ground truth
                boxes to be ignored. Defaults to None.
            proposals (list[mindspore.Tensor], optional): Proposals of each sample.
                Defaults to None.
    ...
    # TODO: merge_aug_bboxes_3d exist NMS GPU op (mmdet3d.core.merge_aug_bboxes_3d)
    ...
    def show_results(self, data, result, out_dir):
        ...
        for batch_id in range(len(result)):
            if common.is_list_of(data['points'][0], ms.Tensor):
                points = data['points'][0][batch_id]
            else:
                ValueError(f"Unsupported data type {type(data['points'][0])} "
                           f'for visualization!')
            if common.is_list_of(data['img_metas'][0], dict):
            ...
            pred_bboxes = pred_bboxes.tensor.asnumpy()
