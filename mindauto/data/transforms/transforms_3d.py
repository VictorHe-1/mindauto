...
class ObjectRangeFilter:
    ...
    def __call__(self, input_dict):
        ...
        if isinstance(input_dict['gt_bboxes_3d'],
            ...
            raise ValueError(f"Not support BBox type {type(input_dict['gt_bboxes_3d'])}")
...
class NormalizeMultiviewImage:
    ...
    def __call__(self, results):
        ...
        results['img'] = [common.imnormalize(img, self.mean, self.std, self.to_rgb) for img in results['img']]
...
class PadMultiViewImage:
    ...
    def _pad_img(self, results):
        ...
        if self.size is not None:
            padded_img = [common.impad(
                img, shape=self.size, pad_val=self.pad_val) for img in results['img']]
        elif self.size_divisor is not None:
            padded_img = [common.impad_to_multiple(
...
class MultiScaleFlipAug3D(object):
    ...
    def __init__(self,
        ...
        assert common.is_list_of(self.flip_direction, str)
