...
class BasePoints(object):
    ...
    Args:
        tensor (ms.Tensor | np.ndarray | list): a N x points_dim matrix.
    ...
    Attributes:
        tensor (ms.Tensor): Float matrix of N x points_dim.
    ...
    def __init__(self, tensor, points_dim=3, attribute_dims=None):
        ...
        if tensor.numel() == 0:
            ...
            tensor = tensor.reshape((0, points_dim)).astype(ms.float32)
        assert tensor.ndim() == 2 and tensor.shape[-1] == \
            points_dim, tensor.shape
    ...
    @property
    def coord(self):
        """ms.Tensor: Coordinates of each point with size (N, 3)."""
    ...
    def coord(self, tensor):
        ...
        except (RuntimeError, ValueError):  # for ms.Tensor and np.ndarray
            raise ValueError(f'got unexpected shape {tensor.shape}')
        if not isinstance(tensor, ms.Tensor):
            tensor = ms.Tensor(tensor, dtype=self.tensor.dtype)
        self.tensor[:, :3] = tensor

    @property
    def height(self):
        """ms.Tensor: A vector with height of each point."""
    ...
    def height(self, tensor):
        ...
        if self.attribute_dims is not None and \
            ...
            self.tensor = ops.cat([self.tensor, tensor.unsqueeze(1)], axis=1)
            self.attribute_dims.update(dict(height=attr_dim))
            self.points_dim += 1

    @property
    def color(self):
        """ms.Tensor: A vector with color of each point."""
        if self.attribute_dims is not None and \
    ...
    def color(self, tensor):
        ...
        if self.attribute_dims is not None and \
            ...
            if self.attribute_dims is None:
                self.attribute_dims = dict()
            attr_dim = self.shape[1]
            self.tensor = ops.cat([self.tensor, tensor], axis=1)
    ...
    def shuffle(self):
        ...
        Returns:
            ms.Tensor: The shuffled index.
        """
        idx = ops.randperm(self.__len__())
    ...
    def rotate(self, rotation, axis=None):
        ...
        Args:
            rotation (float, np.ndarray, ms.Tensor): Rotation matrix
                or angle.
            axis (int): Axis to rotate at. Defaults to None.
        """
        if not isinstance(rotation, ms.Tensor):
            rotation = ms.Tensor(rotation, dtype=self.tensor.dtype)
        ...
        if rotation.numel() == 1:
            rot_sin = ops.sin(rotation)
            rot_cos = ops.cos(rotation)
            if axis == 1:
                rot_mat_T = ms.Tensor([[rot_cos, 0, -rot_sin],
                                      [0, 1, 0],
                                      [rot_sin, 0, rot_cos]], dtype=rotation.dtype)
            elif axis == 2 or axis == -1:
                rot_mat_T = ms.Tensor([[rot_cos, -rot_sin, 0],
                                      [rot_sin, rot_cos, 0],
                                      [0, 0, 1]], dtype=rotation.dtype)
            elif axis == 0:
                rot_mat_T = ms.Tensor([[0, rot_cos, -rot_sin],
                                      [0, rot_sin, rot_cos],
                                      [1, 0, 0]], dtype=rotation.dtype)
            else:
    ...
    def translate(self, trans_vector):
        ...
        Args:
            trans_vector (np.ndarray, ms.Tensor): Translation
    ...
    def in_range_3d(self, point_range):
        ...
        Args:
            point_range (list | ms.Tensor): The range of point
        ...
        Returns:
            ms.Tensor: A binary vector indicating whether each point is \
    ...
    def in_range_bev(self, point_range):
        ...
        Returns:
            ms.Tensor: Indicating whether each point is inside \
                the reference range.
        """
        pass
    ...
    def __getitem__(self, item):
        ...
        Note:
            ...
            3. `new_points = points[vector]`:
                where vector is a ms.BoolTensor with `length = len(points)`.
        ...
        if isinstance(item, int):
            ...
            if self.attribute_dims is not None:
                ...
                attribute_dims = None
        elif isinstance(item, (slice, np.ndarray, ms.Tensor)):
            p = self.tensor[item]
            attribute_dims = self.attribute_dims
        else:
